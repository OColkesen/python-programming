    ...
    def play_game(self, player_x, player_o):
            ...
            while (    (not self.win_for("X"))
               ...
               and (not self.is_full())):
                if current_player == player_o:
                    move = player_o.next_move(self)
                    print("Computer playing for O plays at " + str(move))
                    self.add_move(move, "O")
                else: 
                    move = Board.INVALID_MOVE
                    while not self.allows_move(move):
                        move = int(input("X's move: "))
        ...
        if player_o == "human":
            while (    (not self.win_for("X"))
               ...
               and (not self.is_full())):
                if current_player == player_x:
                    move = player_x.next_move(self)
                    print("Computer playing for X plays at " + str(move))
                    self.add_move(move, "X")
                else: 
                    move = Board.INVALID_MOVE
                    while not self.allows_move(move):
                        move = int(input("O's move: "))
                    self.add_move(move, "O")
            ...
            while (    (not self.win_for("X"))
               ...
               and (not self.is_full())):
                ...
                print("Computer playing for " + side + " plays at " + str(move))
                self.add_move(move, side)
                print()
                print(self)
                print()
